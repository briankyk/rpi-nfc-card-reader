...
dev =evdev.InputDevice('/dev/input/event24')
dev.grab()
capture = '' 
caps = False 
    ...
    if event.type == ecodes.EV_KEY: 
        ...
        if data.keystate == 1: # Down events only 
            ...
            if (data.scancode != 42) and (data.scancode != 28): 
                capture += key_lookup 
            if(data.scancode == 28): 
                print (capture)   # Print captured and translatedcard number 
                hktz = pytz.timezone('Asia/Hong_Kong')
                now = datetime.datetime.now(tz=hktz)
                date_now = now.strftime("%d/%m/%Y")
                time_now = now.strftime("%H:%M:%S")
                data_update = [date_now, time_now, capture]
                sheet.append_row(data_update) 
                pprint(data)
                capture = ''
